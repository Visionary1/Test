...
<!--song 파라미터 -->
<%
String state = request.getParameter("song");
...
<div class="border border-success d-flex mt-3 artist-info">
	...
	<div class="m-3 artist-info">
		<span class="fs-3"><%=artistInfo.get("name")%></span><br> <span><%=artistInfo.get("agency")%></span><br> <span><%=artistInfo.get("debute")%> 데뷔</span>
	</div>
</div>
	...
	<table class="table table-borderless table-hover">
		...
		<tbody>
			...
			<tr>
				<td><%=item.get("id")%></td>
				<td>
					<a href="?song=<%=item.get("title")%>" class=""><%=item.get("title")%></a>
				</td>
...
<%
	...
	/* 곡 존재 유무 확인 */
	boolean contains = musicList.stream().anyMatch((t) -> state.equals(t.get("title")));
	...
	/* 곡 있으면 */
		...
		for (Map<String, Object> item : musicList) {
			/*  곡이랑 일치하는 부분 찾기 */
			if (state.equals(item.get("title"))) {
%>
